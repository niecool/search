 ...
 *  根据词典的叶子分词器
 */
public class CHNLeafSegmenter implements LeafSegmenter {
  // 待处理的分词hit队列
  private List<Hit> penddingHits;
 
  public CHNLeafSegmenter() {
    this.penddingHits = new LinkedList<Hit>();
  }
    ...
    if (CharacterUtil.CHAR_CHINESE == context.getCurrentCharType()
        ...
        for (Hit hit : tmpArray) {
          ...
          if (hit.isMatch()) {
            ...
            Lexeme newLexeme = new Lexeme(context.getBufferOffset(), hit.getBegin(),
                context.getCursor() - hit.getBegin() + 1, hit.getMatchedDictSegment().getWordType(),
                hit.getMatchedDictSegment().getSynonymWordsMap(),
                hit.getMatchedDictSegment().getExtendedWordsMap(),hit.getMatchedDictSegment().getSymbolNum(),true);
      ...
      // *********************************
          ...
          if (singleCharHit.isMatch()) {// 首字成词
            // 输出当前的词
            Lexeme newLexeme = new Lexeme(context.getBufferOffset(), context.getCursor(), 1,singleCharHit.getMatchedDictSegment().getWordType());
            context.addLexeme(newLexeme);
